  ...
  return (
    <div>
      ...
      <div className="checkCodeHomeWork">
        {getStop() === 1 ? <h6>Nhập code của bạn</h6> : <div></div>}
        {getStop() === 1 ? (
          <CodeMirror
            id="source_code"
            value={source}
      ...
      {getStop() === 1 ? (
        <Button id="historyHomeWork" color="info" onClick={toggle}>
          Lịch sử
        </Button>
      ) : (
        <div></div>
      ...
      {getStop() === 1 ? (
        <Button id="clearHomeWork" color="danger" onClick={clear}>
          Xoá code
        </Button>
      ) : (
        <div></div>
