...
const StudyCMenu = () => {
  const handleClick = e => {
    ...
    firebaseApp.database().ref('Lecture/' + localStorage.getItem('emailID') + "/Lecture" + localStorage.getItem('Lecture')).set ({
      isRead: true
   })
  };
  ...
  const setColor = (key) => {
    var isRead;
    firebaseApp.database().ref("Lecture/"+ localStorage.getItem('emailID') + "/Lecture" + key + "/isRead").on('value', function(snapshot) {
      if (snapshot.exists){
        isRead = snapshot.val()
      }
    })
