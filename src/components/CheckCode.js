...
const getInform = (Number) => {
  firebaseApp.database().ref('Test/Test' + Number + "/Title").on('value', function(snapshot) {
    if (snapshot.exists) {
    Title = snapshot.val()
    }
  })
  firebaseApp.database().ref('Test/Test' + Number + "/Introduct").on('value', function(snapshot) {
    if (snapshot.exists) {
    Introduct = snapshot.val()
    }
  })
  firebaseApp.database().ref('Test/Test' + Number + "/Step_test").on('value', function(snapshot) {
    if (snapshot.exists) {
...
const getTestInform = (Number, Step) => {
  ...
  firebaseApp.database().ref('Test/Test' + Number).child("Expected_Output/Expect"+ Step + "/Output").on('value', function(snapshot) {
    if (snapshot.exists){
      Output = snapshot.val()
    }
  })
...
const getHistory = () => {
  firebaseApp.database().ref("historyCode/"+ localStorage.getItem('emailID') + "/Test" + localStorage.getItem('testKey') + "/history").on('value', function(snapshot) {
    if (snapshot.exists){
      codeHistory = decode(snapshot.val())
    }
  })
  firebaseApp.database().ref("historyCode/"+ localStorage.getItem('emailID') + "/Test" + localStorage.getItem('testKey') + "/time").on('value', function(snapshot) {
    if (snapshot.exists){
      time = snapshot.val()
    }
  })
